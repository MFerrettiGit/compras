<#
publish.ps1 - Cria (se preciso) o repo MFerrettiGit/compras, faz commit/push e ativa o GitHub Pages.
...
#>
param(
  [string]$Message = "Atualiza painel de compras",
  [string]$RepoDir = "C:\Users\COMPRASD\compras",
...
$tok = [CredVaultC]::Get("git:https://github.com")
if($tok -eq "FAIL"){ throw "Credencial do GitHub nao encontrada (git:https://github.com)." }
$hdr = @{ Authorization = "token $tok"; "User-Agent" = "ferretti-compras"; Accept = "application/vnd.github+json" }
...
try {
  ...
  Write-Output "Criando repo $Owner/$Repo ..."
  $body = @{ name=$Repo; private=$false; description="Painel de Compras - Estoque e Reposicao (M. Ferretti)"; has_issues=$false; has_wiki=$false } | ConvertTo-Json
